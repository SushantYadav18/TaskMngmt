...
function fetchBaseQuery({ baseUrl, prepareHeaders = (x) => x, fetchFn = defaultFetchFn, paramsSerializer, isJsonContentType = defaultIsJsonContentType, jsonContentType = "application/json", jsonReplacer, timeout: defaultTimeout, responseHandler: globalResponseHandler, validateStatus: globalValidateStatus, ...baseFetchOptions } = {}) {
	...
	return async (arg, api) => {
		...
		if (!config.headers.has("content-type") && isJsonifiable(config.body)) config.headers.set("content-type", jsonContentType);
		if (isJsonifiable(config.body) && isJsonContentType(config.headers)) config.body = JSON.stringify(config.body, jsonReplacer);
		if (params) {
...
function calculateProvidedBy(description, result, error, queryArg, meta, assertTagTypes) {
	if (isFunction(description)) return description(result, error, queryArg, meta).map(expandTagDescription).map(assertTagTypes);
		...
		const queryAction = (arg, { subscribe = true, forceRefetch, subscriptionOptions, [forceQueryFnSymbol]: forceQueryFn } = {}) => (dispatch, getState) => {
			...
			if (!runningQuery && !skippedSynchronously && !forceQueryFn) {
				...
				statePromise.then(() => {
					...
					if (!countObjectKeys$1(running)) runningQueries.delete(dispatch);
				});
			}
			return statePromise;
		};
	...
	function buildInitiateMutation(endpointName) {
		return (arg, { track = true, fixedCacheKey } = {}) => (dispatch, getState) => {
			...
			ret.then(() => {
				...
				if (!countObjectKeys$1(running)) runningMutations.delete(dispatch);
			});
			if (fixedCacheKey) {
	...
	const executeEndpoint = async (arg, { signal, abort, rejectWithValue, fulfillWithValue, dispatch, getState, extra }) => {
		...
		try {
			...
			if (forceQueryFn) result = forceQueryFn();
				...
				if (endpointDefinition.transformResponse) transformResponse = endpointDefinition.transformResponse;
			} else result = await endpointDefinition.queryFn(arg.originalArgs, baseQueryApi, endpointDefinition.extraOptions, (arg2) => baseQuery(arg2, baseQueryApi, endpointDefinition.extraOptions));
			...
			if (catchedError instanceof HandledError) {
				...
				if (endpointDefinition.query && endpointDefinition.transformErrorResponse) transformErrorResponse = endpointDefinition.transformErrorResponse;
				try {
...
In the case of an unhandled error, no tags will be "provided" or "invalidated".`, catchedError);
			else console.error(catchedError);
			throw catchedError;
		}
	};
	...
	const invalidationSlice = createSlice({
		...
		reducers: { updateProvidedBy: {
			reducer(draft, action) {
				...
				for (const tagTypeSubscriptions of Object.values(draft)) for (const idSubscriptions of Object.values(tagTypeSubscriptions)) {
					const foundAt = idSubscriptions.indexOf(queryCacheKey);
					if (foundAt !== -1) idSubscriptions.splice(foundAt, 1);
				}
		...
		extraReducers(builder) {
			builder.addCase(querySlice.actions.removeQueryResult, (draft, { payload: { queryCacheKey } }) => {
				...
				for (const [type, incomingTags] of Object.entries(provided)) for (const [id, cacheKeys] of Object.entries(incomingTags)) {
					const subscribedQueries = (draft[type] ??= {})[id || "__internal_without_id"] ??= [];
					for (const queryCacheKey of cacheKeys) if (!subscribedQueries.includes(queryCacheKey)) subscribedQueries.push(queryCacheKey);
				}
...
function buildSelectors({ serializeQueryArgs, reducerPath, createSelector: createSelector2 }) {
	...
	function buildQuerySelector(endpointName, endpointDefinition) {
		return (queryArgs) => {
			...
			return createSelector2(queryArgs === skipToken ? selectSkippedQuery : selectQuerySubstate, withRequestFlags);
		};
	}
	...
	function selectCachedArgsForQuery(state, queryName) {
		return Object.values(state[reducerPath].queries).filter((entry) => entry?.endpointName === queryName && entry.status !== "uninitialized").map((entry) => entry.originalArgs);
	}
}
...
var defaultSerializeQueryArgs = ({ endpointName, queryArgs }) => {
	...
	if (typeof cached === "string") serialized = cached;
		...
		if (isPlainObject(queryArgs)) cache$1?.set(queryArgs, stringified);
		...
		function injectEndpoints(inject) {
			...
			for (const [endpointName, definition] of Object.entries(evaluatedEndpoints)) {
				if (!inject.overrideExisting && endpointName in context.endpointDefinitions) {
					if (typeof process !== "undefined" && true) console.error(`called \`injectEndpoints\` to override already-existing endpointName ${endpointName} without specifying \`overrideExisting: true\``);
					continue;
				}
...
var buildCacheCollectionHandler = ({ reducerPath, api, context, internalState }) => {
	...
	const handler = (action, mwApi, internalState2) => {
		...
		if (api.util.resetApiState.match(action)) for (const [key, timeout] of Object.entries(currentRemovalTimeouts)) {
		...
		if (context.hasRehydrationInfo(action)) {
			...
			for (const [queryCacheKey, queryState] of Object.entries(queries)) handleUnsubscribe(queryCacheKey, queryState?.endpointName, mwApi, state.config);
		}
	};
	function handleUnsubscribe(queryCacheKey, endpointName, api2, config) {
		...
		if (!anySubscriptionsRemainingForKey(queryCacheKey)) {
			...
			currentRemovalTimeouts[queryCacheKey] = setTimeout(() => {
				if (!anySubscriptionsRemainingForKey(queryCacheKey)) api2.dispatch(removeQueryResult({ queryCacheKey }));
...
var buildInvalidationByTagsHandler = ({ reducerPath, context, context: { endpointDefinitions }, mutationThunk, queryThunk, api, assertTagType, refetchQuery, internalState }) => {
	...
	function hasPendingRequests(state) {
		for (const key in state.queries) if (state.queries[key]?.status === "pending") return true;
		for (const key in state.mutations) if (state.mutations[key]?.status === "pending") return true;
		return false;
	}
	function invalidateTags(newTags, mwApi) {
		...
		context.batch(() => {
			...
			for (const { queryCacheKey } of valuesArray) {
				...
				if (querySubState) {
					if (countObjectKeys$1(subscriptionSubState) === 0) mwApi.dispatch(removeQueryResult({ queryCacheKey }));
					else if (querySubState.status !== "uninitialized") mwApi.dispatch(refetchQuery(querySubState, queryCacheKey));
				}
			}
		});
	}
...
var buildPollingHandler = ({ reducerPath, queryThunk, api, refetchQuery, internalState }) => {
	const currentPolls = {};
	const handler = (action, mwApi) => {
		if (api.internalActions.updateSubscriptionOptions.match(action) || api.internalActions.unsubscribeQueryResult.match(action)) updatePollingInterval(action.payload, mwApi);
		if (queryThunk.pending.match(action) || queryThunk.rejected.match(action) && action.meta.condition) updatePollingInterval(action.meta.arg, mwApi);
		...
		if (api.util.resetApiState.match(action)) clearPolls();
	};
	function startNextPoll({ queryCacheKey }, api2) {
		...
		currentPolls[queryCacheKey] = {
			...
			timeout: setTimeout(() => {
				if (state.config.focused || !skipPollingIfUnfocused) api2.dispatch(refetchQuery(querySubState, queryCacheKey));
	...
	function clearPolls() {
		for (const key of Object.keys(currentPolls)) cleanupPollForKey(key);
	}
	function findLowestPollingInterval(subscribers = {}) {
		let skipPollingIfUnfocused = false;
		let lowestPollingInterval = Number.POSITIVE_INFINITY;
		for (let key in subscribers) if (!!subscribers[key].pollingInterval) {
	...
	function refetchValidQueries(api2, type) {
		...
		context.batch(() => {
			for (const queryCacheKey of Object.keys(subscriptions)) {
				...
				if (Object.values(subscriptionSubState).some((sub) => sub[type] === true) || Object.values(subscriptionSubState).every((sub) => sub[type] === void 0) && state.config[type]) {
					if (countObjectKeys$1(subscriptionSubState) === 0) api2.dispatch(removeQueryResult({ queryCacheKey }));
					else if (querySubState.status !== "uninitialized") api2.dispatch(refetchQuery(querySubState, queryCacheKey));
				}
			}
		});
	}
...
var buildCacheLifecycleHandler = ({ api, reducerPath, context, queryThunk, mutationThunk, internalState }) => {
	...
	const handler = (action, mwApi, stateBefore) => {
		...
		if (queryThunk.pending.match(action)) {
			...
			if (!oldState && state) handleNewKey(action.meta.arg.endpointName, action.meta.arg.originalArgs, cacheKey, mwApi, action.meta.requestId);
		...
		} else if (api.util.resetApiState.match(action)) for (const [cacheKey2, lifecycle] of Object.entries(lifecycleMap)) {
...
var buildBatchedActionsHandler = ({ api, queryThunk, internalState }) => {
	...
	const actuallyMutateSubscriptions = (mutableState, action) => {
		if (updateSubscriptionOptions.match(action)) {
			...
			if (mutableState?.[queryCacheKey]?.[requestId]) mutableState[queryCacheKey][requestId] = options;
			return true;
		}
		if (unsubscribeQueryResult.match(action)) {
			const { queryCacheKey, requestId } = action.payload;
			if (mutableState[queryCacheKey]) delete mutableState[queryCacheKey][requestId];
			return true;
		}
		...
		if (queryThunk.pending.match(action)) {
			...
			if (arg.subscribe) substate[requestId] = arg.subscriptionOptions ?? substate[requestId] ?? {};
			return true;
		}
		let mutated = false;
	...
	const getSubscriptionCount = (queryCacheKey) => {
		return countObjectKeys$1(getSubscriptions()[queryCacheKey] ?? {});
	};
	const isRequestSubscribed = (queryCacheKey, requestId) => {
		return !!getSubscriptions()?.[queryCacheKey]?.[requestId];
	};
	...
	return (action, mwApi) => {
		if (!previousSubscriptions) previousSubscriptions = JSON.parse(JSON.stringify(internalState.currentSubscriptions));
...
function buildMiddleware(input) {
	...
	const middleware = (mwApi) => {
		...
		let internalState = { currentSubscriptions: {} };
		const builderArgs = {
			...input,
			internalState,
			refetchQuery,
		...
		return (next) => {
			return (action) => {
				...
				if (!!mwApi.getState()[reducerPath]) {
					...
					if (isThisApiSliceAction(action) || context.hasRehydrationInfo(action)) for (let handler of handlers) handler(action, mwApiWithNext, stateBefore);
				}
				return res;
			};
		};
	...
	init(api, { baseQuery, tagTypes, reducerPath, serializeQueryArgs, keepUnusedDataFor, refetchOnMountOrArgChange, refetchOnFocus, refetchOnReconnect, invalidationBehavior }, context) {
		...
		return {
			...
			injectEndpoint(endpointName, definition) {
				...
				if (isQueryDefinition$1(definition)) safeAssign$1(anyApi.endpoints[endpointName], {
					name: endpointName,
					select: buildQuerySelector(endpointName, definition),
					initiate: buildInitiateQuery(endpointName, definition)
				}, buildMatchThunkActions(queryThunk, endpointName));
				else if (isMutationDefinition$1(definition)) safeAssign$1(anyApi.endpoints[endpointName], {
					name: endpointName,
...
function useStableQueryArgs(queryArgs, serialize, endpointDefinition, endpointName) {
	...
	(0, import_react.useEffect)(() => {
		if (cache2.current.serialized !== incoming.serialized) cache2.current = incoming;
	}, [incoming]);
...
function useShallowStableValue(value) {
	...
	(0, import_react.useEffect)(() => {
		if (!shallowEqual(cache2.current, value)) cache2.current = value;
	}, [value]);
	return shallowEqual(cache2.current, value) ? cache2.current : value;
}
var cache = WeakMap ? /* @__PURE__ */ new WeakMap() : void 0;
var defaultSerializeQueryArgs$1 = ({ endpointName, queryArgs }) => {
	let serialized = "";
...
function buildHooks({ api, moduleOptions: { batch, hooks: { useDispatch, useSelector, useStore }, unstable__sideEffectsInRender, createSelector: createSelector2 }, serializeQueryArgs, context }) {
    ...
    You must add the middleware for RTK-Query to function correctly!`);
			...
			const stableArg = useStableQueryArgs(skip ? skipToken : arg, defaultSerializeQueryArgs$1, context.endpointDefinitions[name], name);
...
refetch: () => {
		...
		const useQueryState = (arg, { skip = false, selectFromResult } = {}) => {
			...
			const selectDefaultResult = (0, import_react.useMemo)(() => createSelector2([
				select(stableArg),
				(_, lastResult) => lastResult,
				(_) => stableArg
			], queryStatePreSelector, { memoizeOptions: { resultEqualityCheck: shallowEqual } }), [select, stableArg]);
			const querySelector = (0, import_react.useMemo)(() => selectFromResult ? createSelector2([selectDefaultResult], selectFromResult, { devModeChecks: { identityFunctionCheck: "never" } }) : selectDefaultResult, [selectDefaultResult, selectFromResult]);
	...
	function buildMutationHook(name) {
		return ({ selectFromResult, fixedCacheKey } = {}) => {
			...
			const currentState = useSelector((0, import_react.useMemo)(() => selectFromResult ? createSelector2([selectDefaultResult], selectFromResult) : selectDefaultResult, [selectFromResult, selectDefaultResult]), shallowEqual);
...
var reactHooksModule = ({ batch: batch$1 = batch, hooks = {
	...
	useSelector,
	useStore
}, createSelector: createSelector2 = createSelector, unstable__sideEffectsInRender = false, ...rest } = {}) => {
	{
		const hookNames = [
			"useDispatch",
			"useSelector",
...
//#endregion
export { ApiProvider, QueryStatus, buildCreateApi, copyWithStructuralSharing, coreModule, coreModuleName, createApi, defaultSerializeQueryArgs, fakeBaseQuery, fetchBaseQuery, reactHooksModule, reactHooksModuleName, retry, setupListeners, skipToken };
