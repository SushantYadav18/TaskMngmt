...
function each(obj, iter) {
	if (getArchtype(obj) === 0) Object.entries(obj).forEach(([key, value]) => {
		iter(key, value, obj);
	});
...
function shallowCopy(base, strict) {
	...
	if (!strict && isPlainObject(base)) {
		if (!getPrototypeOf(base)) return Object.assign(/* @__PURE__ */ Object.create(null), base);
		return { ...base };
	}
...
function isPending(...asyncThunks) {
	...
	return (action) => {
		return isAnyOf(...asyncThunks.map((asyncThunk) => asyncThunk.pending))(action);
	};
}
function isRejected(...asyncThunks) {
	...
	return (action) => {
		return isAnyOf(...asyncThunks.map((asyncThunk) => asyncThunk.rejected))(action);
	};
}
function isRejectedWithValue(...asyncThunks) {
	...
	if (asyncThunks.length === 0) return (action) => {
		return isAllOf(isRejected(...asyncThunks), hasFlag)(action);
	};
...
function isFulfilled(...asyncThunks) {
	...
	return (action) => {
		return isAnyOf(...asyncThunks.map((asyncThunk) => asyncThunk.fulfilled))(action);
	};
}
function isAsyncThunkAction(...asyncThunks) {
	...
	return (action) => {
		...
		return isAnyOf(...matchers)(action);
	};
}
...
var addListener = Object.assign(createAction(`${alm}/add`), { withTypes: () => addListener });
var clearAllListeners = createAction(`${alm}/removeAll`);
var removeListener = Object.assign(createAction(`${alm}/remove`), { withTypes: () => removeListener });
...
var createListenerMiddleware = (middlewareOptions = {}) => {
	...
	Object.assign(startListening, { withTypes: () => startListening });
	...
	Object.assign(stopListening, { withTypes: () => stopListening });
